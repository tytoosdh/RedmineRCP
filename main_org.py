...
def print_sel(e):
    ...
    print(data_wybrana)
    timeEntryTable(conn, tabelaFrame, getCurrentUserId(conn), data_wybrana, root)
...
def timeEntryTable(conn, ws, userid, data, root):
    ...
    list.column("Projekt", anchor=CENTER, width=200)
    list.column("Zadanie", anchor=CENTER, width=200)
    list.column("Opis działania", anchor=CENTER, width=200)
    list.column("Czas", anchor=CENTER, width=100)

    list.heading("#0", text="", anchor=CENTER)
    list.heading("Projekt", text="Projekt", anchor=CENTER)
    list.heading("Zadanie", text="Projekt", anchor=CENTER)
    list.heading("Opis działania", text="Opis działania", anchor=CENTER)
    list.heading("Czas", text="Czas", anchor=CENTER)
    ...
    for time_entry in time_entries:
        ...
        list.insert(parent='', index='end', iid=iid, text='',
                values=(project, issue_name, comments, hours))
        suma = suma + time_entry.hours
        iid = iid + 1
...
dzialanieFrame = Frame(root)
dzialanieFrame.pack(side=TOP)

dzialanie2Frame = Frame(root)
dzialanie2Frame.pack(side=TOP)

dzialanie2Frame = Frame(root)
dzialanie2Frame.pack(side=TOP)

aktywnoscFrame = Frame(root)
...
opis = StringVar()
entry = Entry(dzialanie2Frame, textvariable=opis, width=100, borderwidth=2).pack(side=LEFT, pady=5)
